...
//      cerr << i16 << " " << i32 << " " << j1 << " " << j2 << " " << ((j1 < ni)?i1[j1]:-1) << " " << ((j2 < ni)?i2[j2]:-1) << endl;
      ...
      if (j1 < ni-1 && i1[j1] >= i1[j1+1])
         error("DEBUG: stri__UChar16_to_UChar32_index");
#endif
         i1[j1] = i32 + adj1;
         ++j1;
      }
...
#define STRI__BINPROP_TEST(x, y) \
   ...
   // we were considering putting a finite state automata here,
   // but the binary properties' names are too diverse:
   // there are, e.g. 22 distinct first letters
   // => the speed improvements would be small
   
   // instead of that, we'll use the binary search algorithm
   
   static const char* binprop_names[] = {
   ...
   static const UProperty binprop_code[] = {
