...
test_that("stri_locate_last_words", {
   ...
   expect_equivalent(stri_locate_last_words(c("ala ma kota", "    kot", "   ", "", NA, "a", "aa ", "aa")),
      matrix(c(8,11,5,7,NA,NA,NA,NA,NA,NA,1,1,1,2,1,2), ncol=2, byrow=TRUE))
   expect_equivalent(stri_locate_last_words(character(0)),
      matrix(integer(0), ncol=2, byrow=TRUE))
...
test_that("stri_locate_all_words", {
   ...
   expect_identical(colnames(stri_locate_all_words("stringi")[[1]]), c("start", "end"))

   expect_equivalent(stri_locate_all_words(c("    \t   ", "stri\u0105gi", NA)),
