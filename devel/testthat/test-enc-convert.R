...
test_that("stri_encode", {
    ...
    expect_equivalent(charToRaw(stri_encode("ąa", "", "cp1250")), as.raw(c(185, 
        97)))
    ...
    if (!stri_info()$ICU.UTF8) {
        ...
        expect_equivalent(stri_encode(c("a", "\xb1", NA, "ą"), "", "UTF-8"), c("a", 
            "ą", NA, "ą"))
        expect_equivalent(stri_encode(c("a", "\xb1", NA, "ą"), "", ""), c("a", "\xb1", 
            NA, "\xb1"))
    ...
    if (!stri_info()$ICU.UTF8) {
        ...
        expect_equivalent(stri_encode(c("a", "\xb9", NA, "ą"), NULL, "UTF-8"), c("a", 
            "ą", NA, "ą"))
        expect_equivalent(stri_encode(c("a", "\xb9", NA, "ą")), c("a", "\xb9", NA, 
            "\xb9"))
        suppressMessages(stri_enc_set(defenc))
    }
...
test_that("stri_enc_fromutf32", {
    ...
    expect_identical(stri_enc_fromutf32(c(43, 234, 649, 63, 23532, 23632)), intToUtf8(c(43, 
        234, 649, 63, 23532, 23632)))
    expect_warning(stri_enc_fromutf32(list(c(43, 234, 649, 63, 23532, 233643642), 
        65)))
...
test_that("stri_enc_toascii", {
    ...
    expect_identical(stri_enc_toascii(c("sgajhgaoi", NA, "ąfds吲Ą")), c("sgajhgaoi", 
        NA, "\032fds\032\032"))
