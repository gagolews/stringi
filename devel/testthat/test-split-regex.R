...
test_that("stri_split_regex", {
    ...
    expect_identical(stri_split_regex("ala ma kota 1 a", "[a-z] [a-z]"), list(c("al", 
        "", "ota 1 a")))
    expect_identical(stri_split_regex("ala ma kota 1 a", "[a-z] [a-z]*"), list(c("al", 
        " kot", "1 a")))
    expect_identical(stri_split_regex("ala ma kota 1 a", "[a-z] [a-z]+"), list(c("al", 
        " kota 1 a")))
    expect_identical(stri_split_regex("ala ma kota 1 a", "[a-z] [1-9]"), list(c("ala ma kot", 
        " a")))
    expect_identical(stri_split_regex("ala ma kota 1 a", "[a-z] [1-9]+"), list(c("ala ma kot", 
        " a")))
    
    
    expect_identical(stri_split_regex(c("ąĆć", "ąć"), "Ć*"), list(c("", "ą", 
        "", "ć", ""), c("", "ą", "ć", "")))  # match of zero length
    expect_identical(stri_split_regex(c("ąĆć", "ąć"), "(?<=Ć)"), list(c("ąĆ", 
        "ć"), "ąć"))  # match of zero length:
    ...
    expect_identical(stri_split_regex("123-456-789", "-", n = 1:3), list(c("123-456-789"), 
        c("123", "456-789"), c("123", "456", "789")))
    expect_identical(stri_split_regex("123-456-789", "[1-8]-.", n = 5), list(c("12", 
        "5", "89")))
    
    # tokens_only
    expect_identical(stri_split_regex("a_b_c_d", "_"), list(c("a", "b", "c", "d")))
    expect_identical(stri_split_regex("a_b_c__d", "_"), list(c("a", "b", "c", "", 
        "d")))
    expect_identical(stri_split_regex("a_b_c__d", "_", omit_empty = TRUE), list(c("a", 
        "b", "c", "d")))
    expect_identical(stri_split_regex("a_b_c__d", "_", n = 2, tokens_only = FALSE), 
        list(c("a", "b_c__d")))
    expect_identical(stri_split_regex("a_b_c__d", "_", n = 2, tokens_only = TRUE), 
        list(c("a", "b")))
    expect_identical(stri_split_regex("a_b_c__d", "_", n = 4, omit_empty = TRUE, 
        tokens_only = TRUE), list(c("a", "b", "c", "d")))
    expect_identical(stri_split_regex("a_b_c__d", "_", n = 4, omit_empty = FALSE, 
        tokens_only = TRUE), list(c("a", "b", "c", "")))
    expect_identical(stri_split_regex(c("ab_c", "d_ef_g", "h", ""), "_", n = 1, tokens_only = TRUE, 
        omit_empty = TRUE), list("ab", "d", "h", character(0)))
    expect_identical(stri_split_regex(c("ab_c", "d_ef_g", "h", ""), "_", n = 2, tokens_only = TRUE, 
        omit_empty = TRUE), list(c("ab", "c"), c("d", "ef"), "h", character(0)))
    expect_identical(stri_split_regex(c("ab_c", "d_ef_g", "h", ""), "_", n = 3, tokens_only = TRUE, 
