...
test_that("stri_extract_words", {
   ...
   expect_identical(stri_extract_words(c("   ", "", NA), omit_no_match = TRUE),
      list(character(0), character(0), NA_character_))
   
   expect_identical(stri_extract_words(c("", "ala", "ma kota"), simplify=TRUE),
      matrix(c(NA, NA, "ala", NA, "ma", "kota"), nrow=3, byrow=TRUE))
   
   expect_identical(stri_extract_words(c("", "ala", "ma kota"), simplify=TRUE, omit_no_match=TRUE),
      matrix(c(NA, NA, "ala", NA, "ma", "kota"), nrow=3, byrow=TRUE))
