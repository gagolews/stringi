...
for (test_i in seq_along(alltests)) {
   ...
   rngy <- c(min(sapply(results_data, function(data) data$q1.time)),
             max(sapply(results_data, function(data) data$q3.time)))
   ...
   for (i in seq_along(results_data)) {
      ...
      git_sha <- unique(data$git_sha)
      datetime <- unique(as.character(data$datetime))
