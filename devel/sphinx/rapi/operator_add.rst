...
These operators act like a call to `stri_join(e1, e2, sep='')`_. However, note that joining 3 vectors, e.g., ``e1 %s+% e2 %s+% e3`` is slower than `stri_join(e1, e2, e3, sep='')`_, because it creates a new (temporary) result vector each time the operator is applied.
...
Other join: `stri_dup()`_, `stri_flatten()`_, `stri_join_list()`_, `stri_join()`_
...
.. _stri_join(e1, e2, sep=''): stri_join.html
.. _stri_join(e1, e2, e3, sep=''): stri_join.html
