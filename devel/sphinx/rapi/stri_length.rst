...
If a given string is in UTF-8 and has not been properly normalized (e.g., by `stri_trans_nfc`_), the returned counts may sometimes be misleading. See `stri_count_boundaries`_ for a method to count *Unicode characters*. Moreover, if an incorrect UTF-8 byte sequence is detected, then a warning is generated and the corresponding output element is set to ``NA``, see also `stri_enc_toutf8`_ for a method to deal with such cases.
...
Other length: `stri_isempty()`_, `stri_numbytes()`_, `stri_width()`_
