...
\Shorttitle{\pkg{stringi}: Fast and Portable Character String Processing in \proglang{R}}
...
\Keywords{\pkg{stringi}, character strings, text, \pkg{ICU}, Unicode, regular expressions, data cleansing, natural language processing, \proglang{R}}
\Plainkeywords{stringi, character strings, text, ICU, Unicode, regular expressions, data cleansing, natural language processing, R}

\Address{
  \textbf{Marek Gagolewski}\\
  School of Information Technnology\\
  Deakin University\\
...
% knowledge
% like extracting and formatting numbers, parsing and manipulating date/time data
% own/custom file formats (e.g., somewhat broken csv files)
% reporting
% cite knitr - report generation?
...
\item        completes \proglang{R}'s string handling functions with useful functions from other programming languages.
\end{itemize}
\end{quote}
...
around its base \proglang{R} counterparts,
...
%
% A call to \code{extSoftVersion()} reports on the versions of third-party software  used by \proglang{R}.
...
the \pkg{stringi} package (pronounced ``stringy'', IPA \textipa{[{stringi}]}).
...
Section~\ref{Sec:regex} details pattern matching with \pkg{ICU} regular expressions.
...
\noindent
Here we describe \pkg{stringi} 1.5.2,
...
In order to get started with string processing in the \proglang{R}
...
are generated by \code{stri_sub_all()}, \code{stri_extract_all()}, and similar functions.
...
The \code{stri_sub()}  function is of course vectorised with respect to all its arguments
...
The result is a list of character vectors, as each string in the \textit{haystack}
...
\begin{itemize}
\item {ERE}\footnote{Via the \pkg{TRE} library (\url{https://github.com/laurikari/tre/}).} (\textit{extended regular expressions} that conform to the \pkg{POSIX.2-1992} standard);
used by default,
\item {PCRE}\footnote{Via the \pkg{PCRE2} library (\url{https://www.pcre.org/}).} (\textit{\proglang{Perl}-compatible regular expressions}),
in use if \code{perl = TRUE} is set.
\end{itemize}
Other matchers are implemented in the \pkg{ore} (via the \pkg{Onigmo} library)
and \pkg{re2r} (\pkg{RE2}) packages.
...
implemented in \pkg{ICU}, which was inspired by  \code{java.util.regex}
...
dynamically generating reports (compare \pkg{knitr}'s
\code{results = "{}asis"{}} chunk option), or creating text files
