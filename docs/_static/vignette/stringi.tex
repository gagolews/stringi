% makeme:
%     build: xelatex -file-line-error -halt-on-error -shell-escape -interaction=errorstopmode ${FILE_NAME} && bibtex  ${FILE_NAME/.tex/.aux}


% kate: indent-width 2; word-wrap-column 74

% knitr::knit("stringi_v3.Rnw", tangle=TRUE)
%  remove echo=FALSE and add section titles
...
of text operations.
In particular, base \proglang{R} \citep{Rproject:home}
...
available in \pkg{stringr} at that time is given in Table~\ref{Tab:oldstringr}).
...
the open source \pkg{stringi} package (pronounced ``stringy'', IPA \textipa{[{stringi}]}).
...
which are plentiful, \pkg{stringi} relies
...
Services not covered by \pkg{ICU} were implemented from scratch to
...
the most noteworthy facilities provided by \pkg{stringi}
...
As such operations are performed by the very \pkg{ICU} itself,
...
% vectorisation, deep recycling rule and when it is useful (compare section 4.2)
...
\item Section~\ref{Sec:collator} deals with the locale-aware \pkg{ICU} Collator,
...
% We show how to specify locales so that we can exactly pinpoint
...
the (high) performance of \pkg{stringi}, its C++ API, and
...
and in-depth reference manual available by calling \code{help(package="stringi")},
...
fetch the object of our focus: here we describe \pkg{stringi} 1.5.3,
...
First, we load and parse the \proglang{HTML} file
...
generated  with \code{Sweave()} or \pkg{knitr}; see \citealp{knitr}),
...
Again, let us note that no explicit \code{for}/\code{while} loops are necessary.
...
but avoids writing \code{x} twice (convenient when \code{x}
...
Oftentimes, a variable number of
...
of capture groups:
\code{stri\_match\_first\_\-regex()},
...
\code{help("locales")}, e.g., we have \strq{Polish\_Poland} on Windows
...
Moreover, let us note the effect of setting the \code{numeric} option on the
...
In base \proglang{R}, dealing with temporal data in languages
...
Let us stress again that many of the package features are provided by \pkg{ICU}, which
...
\proglang{C} and \proglang{C++}. What is more, the operations that do not rely on \pkg{ICU}
services
...
speed up their projects are kindly referred to the \pkg{ExampleRcppStringi}
package available at \url{https://github.com/gagolews/ExampleRcppStringi}
for an illustration using \pkg{Rcpp} \citep{rcppbook}.
...
\proglang{R} extension whose user base is steadily growing.
...
% PyICU
Moreover, further extension of \pkg{stringi}'s API shall be conveyed
