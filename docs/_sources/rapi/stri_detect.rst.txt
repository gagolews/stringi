...
See also `stri_startswith`_ and `stri_endswith`_ for testing whether a string starts or ends with a match to a given pattern. Moreover, see `stri_subset`_ for a character vector subsetting.
...
Other search_detect: `about_search`_, `stri_startswith()`_
...
.. _stri_endswith: stri_startsendswith.html
