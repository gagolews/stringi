...
local({
   ...
   print(microbenchmark(stri_detect_regex(s, "^[A-Z]"), str_detect(s, "^[A-Z]")))
   print(microbenchmark(stri_detect_regex(srep, "^[A-Z]"), str_detect(srep, "^[A-Z]")))
   ...
   srep2 <- rep(s, 10)
   prep <- rep(p, 60)
   print(microbenchmark(stri_detect_regex(s, p), str_detect(s, p)))
   print(microbenchmark(stri_detect_regex(srep2, p), str_detect(srep2, p)))
   print(microbenchmark(stri_detect_regex(s, prep), str_detect(s, prep)))
